...
import tracking_pg  # local tracker for database operations

# Load environment variables from .env file
load_dotenv()

# Facebook API credentials and dry run flag
...
# ----------------------------- helpers -----------------------------
def upload(tmp: str, title: str, desc: str, ts: int) -> str:
    """
    Uploads a video file to Facebook or simulates the upload if DRY_RUN is enabled.
    Args:
        tmp: Path to the video file.
        title: Video title.
        desc: Video description.
        ts: Scheduled publish time as a Unix timestamp.
    Returns:
        The Facebook video ID (or a fake ID if DRY_RUN).
    """
    if DRY_RUN:
        # Simulate upload and return a fake video ID
        fake = f"DRV_{int(time.time())}"
        log.info("[DRY] upload %s to %s at %d", tmp, FB_PAGEID, ts)
        return fake
    # Perform actual upload to Facebook Graph API
    r = requests.post(
...
def get_queue_row() -> Dict[str, Any]:
    """
    Fetches the first row from the video_to_be_posted_3 table.
    Returns:
...
def main():
    """
    Main function:
    - Fetches a video from the queue.
    - Uploads or schedules it to Facebook.
    - Tracks the repost in the Postgres database.
    """
    row = get_queue_row()
    ...
    sched_dt = (
        datetime.fromisoformat(sched).astimezone(timezone.utc)
        if sched else datetime.now(timezone.utc)
    )
    # Upload or schedule the video to Facebook
